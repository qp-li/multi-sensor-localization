...
class MesNet(torch.nn.Module):
        ...
        def forward(self, u, iekf):
            ...
            measurements_covs = (iekf.cov0_measurement.unsqueeze(0) * (50**z_cov_net))
...
class TORCHIEKF(torch.nn.Module, NUMPYIEKF):
    ...
    def __init__(self, parameter_class=None):
        ...
        NUMPYIEKF.__init__(self, parameter_class=None)  #继承了NUMPYIEKF
    ...
    def state_and_cov_update(Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P, H, r, R):
        ...
        Kt, _ = torch.solve(P.mm(H.t()).t(), S)  #新版pytorch是solve
        K = Kt.t()
        dx = K.mv(r.view(-1))
    ...
    def forward_nets(self, u):
        ...
        return measurements_covs    #IMU帧数 * 2
    ...
    def load(self, args, dataset):
        path_iekf = os.path.join(args.path_temp, "iekfnets_best.p")
        if os.path.isfile(path_iekf):
            mondict = torch.load(path_iekf)   #加载预训练模型
