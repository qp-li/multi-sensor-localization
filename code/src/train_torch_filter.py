...
def prepare_filter(args, dataset):
    iekf = TORCHIEKF()  #实例化一个TORCHIEKF类的对象(但是继承了IEKF)，并初始化__init__

    # set dataset parameter  设置参数，训练和测试都有
    ...
    if args.continue_training:
        iekf.load(args, dataset)    #加载预训练模型
    iekf.train()
...
def mini_batch_step(dataset, dataset_name, iekf, list_rpe, t, ang_gt, p_gt, v_gt, u, N0):
    ...
    Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i = iekf.run(t, u,measurements_covs,
                                                            v_gt, p_gt, t.shape[0],
                                                            ang_gt[0])
    delta_p, delta_p_gt = precompute_lost(Rot, p, list_rpe, N0)   #关键一步，计算loss
    if delta_p is None:
        return -1
...
def precompute_lost(Rot, p, list_rpe, N0):
    N = p.shape[0]  #数据结果的个数
    # Rot_10_Hz = Rot[::10]
    # p_10_Hz = p[::10]  #起点为索引0，索引【+10】取数，即按间隔0.1采样
    Rot_10_Hz = Rot
    p_10_Hz = p
