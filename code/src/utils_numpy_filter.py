...
class NUMPYIEKF:
    ...
    def run(self, t, u, measurements_covs, v_mes, p_mes, N, ang0, dataset_name):
        dt = t[1:] - t[:-1]  # (s)
        if N is None:
            if dataset_name in ['0000', '0004', '0005', '0006']:   N = u.shape[0]   #IMU 的数据帧量
        Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P = self.init_run(dt, u, p_mes, v_mes,
                                       ang0, N)
        #####################添加Fast_lio的结果##########
        if dataset_name in ['0004']:
            pose_index = '/media/wuhan2021/ZX1-grey/AI-IMU data/kitti_360_drive/oxts_10Hz/10Hz_duiqi/0004_odom/time_index.txt'
            pose = '/media/wuhan2021/ZX1-grey/AI-IMU data/kitti_360_drive/oxts_10Hz/10Hz_duiqi/0004_odom/odom-poses.txt'
            # pose = '/media/wuhan2021/ZX1-grey/3.Results/1.multi-sensor/ATE-RTE/GT-poses/' + dataset_name + '.txt'
            pose_index = np.fromfile(pose_index, dtype=int, sep='\n')
            # pose = np.fromfile(pose, dtype=float, sep='\n').reshape(pose_index.shape[0], -1)
            pose = np.fromfile(pose, dtype=float, sep='\n').reshape(-1, 12)
            pose1 = pose[:,[3,7,11]]
            j = 0
        # P是状态量对应的方差[21,21]
        for i in range(1, N):   #初始化后，开始从第1个 遍历迭代
            ...
            # if dataset_name in ['0000', '0004', '0005', '0006']:
            if dataset_name in ['0004']:
                if j < len(pose1) and i == pose_index[j]:
                    pose_fast_lio = pose1[j]
                    j = j + 1
                    print('j:', j)
                    # p[i] = pose_fast_lio
                    p[i] = self.update_fast_lio2(p[i], pose_fast_lio, P) #松耦合
                # p[i] = self.update_fast_lio2(p[i], pose1[i], P)   #和真值一一对应的话用这行，不用判断
    ...
    def propagate_cov(self, P_prev, Rot_prev, v_prev, p_prev, b_omega_prev,
                      b_acc_prev, u, dt):
        F = np.zeros((self.P_dim, self.P_dim)) # 21*21
        G = np.zeros((self.P_dim, self.Q_dim)) # 21*18
    ...
    def update_fast_lio2(self, p, pose_fast_lio, P):
        # H = np.zeros((3, self.P_dim))
        H = np.eye(3)

        R = np.diag([10e-8, 10e-8, 10e-8])
        #构造观测向量  仅为P_fast
        pose_fast_lio = pose_fast_lio

        #计算残差
        y = pose_fast_lio - H.dot(p)

        #计算残差协方差
        S = H @ P[6:9, 6:9] @ H.T + R

        #计算卡尔曼增益
        K = P[6:9, 6:9] @ H.T @ np.linalg.inv(S)
    ...
    def update(self, Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P, u, i, measurement_cov):
        ...
        H = np.zeros((2, self.P_dim))  # 观测为2维，所以2*21
        H[:, 3:6] = Rot_body.T[1:]
        H[:, 15:18] = H_v_imu[1:]
        H[:, 9:12] = H_t_c_i[1:]
        H[:, 18:21] = -Omega[1:]
        r = - v_body[1:]  #观测残差：侧向和垂直方向的速度
        R = np.diag(measurement_cov)   #观测的协方差R
    ...
    def state_and_cov_update(Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P, H, r, R):
        S = H.dot(P).dot(H.T) + R    #
        K = (np.linalg.solve(S, P.dot(H.T).T)).T  # 21*2
        dx = K.dot(r)
        #调整量
        dR, dxi = NUMPYIEKF.sen3exp(dx[:9])   #前9个是R_3,V_3,P_3
        dv = dxi[:, 0]
        dp = dxi[:, 1]
        Rot_up = dR.dot(Rot)
        v_up = dR.dot(v) + dv
        ...
        I_KH = NUMPYIEKF.IdP - K.dot(H)  #  1 - K*H
